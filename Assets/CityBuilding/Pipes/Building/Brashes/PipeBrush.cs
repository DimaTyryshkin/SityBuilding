...
namespace Game.Building
{
	...
	{
		readonly PipeBuildingInfo pipes;
		readonly CrossBuildingInfo cross;
		...
		readonly List<Vector2Int> line;

		public PipeBrush(
			PipeBuildingInfo pipes,
			CrossBuildingInfo cross,
			BrushBuilder brushBuilder
		)
		{
			Assert.IsNotNull(pipes);
			Assert.IsNotNull(cross);
			Assert.IsNotNull(brushBuilder);
			this.pipes = pipes;
			this.cross = cross;
			data = brushBuilder;
			cast = new CellCast();
		...
		public override void LateUpdate()
		{
			...
			data.cellMarkerAvailable.transform.position = data.grid.CellToWorldPoint(cell);
			...
			if (Input.GetMouseButtonDown(2))
			{
				...
				cast.entities.ToStringMultilineAndLog("Cast", x => x.gameObject.name);
			}
		}
		...
		{ 
			...
			for (int i = 0; i < inputLine.Count; i++)
			{
				...
				data.mapBuilder.CastNonAllocate(inputLine[i], ref cast);
				MapElement entity = cast.IsFree ? null : cast.entities[0];
		...
		bool BuildSegment(List<Vector2Int> cells, MapElement from, MapElement to, bool isInverse = false)
		{
			...
			if (!from)
			{
				...
				fromAction.action = () => pipe = pipes.InstantiateNew(cells.ToList(), data.mapBuilder);
			}
			...
			{  
				...
				if(isMiddle)
				{
					...
					fromAction.action = () =>
					{
						...
						var newCross = cross.InstantiateNew(cells[0], data.mapBuilder);
						newCross.AddInPipe(oldPipe);
						newCross.AddOutPipe(newPipe);
						pipe = pipes.InstantiateNew(cells.ToList(), data.mapBuilder);
			...
			if (to is PipeQueue oldPipe2)
			{
				...
				if(isMiddle)
				{
					...
					toAction.action = () =>
					{
						...
						var newCross = cross.InstantiateNew(cells[^1], data.mapBuilder);
		...
		PipeQueue SplitPipe(PipeQueue pipe, Vector2Int cell)
		{
			...
			pipe.cells = first;
			var newPipe = pipes.InstantiateNew(second, data.mapBuilder);
		...
		void RemoveCellFromPipe(PipeQueue pipe, Vector2Int cell)
		{
			...
			if (second.Count >= 2)
			{
				PipeQueue newPipe = pipes.InstantiateNew(second, data.mapBuilder);
				outElement?.AddInPipe(newPipe);
				newPipe.OnChanged();
			}
		}
